...
# problème. "currency" = devise de facturation du marketplace ; SE et PL ne
# facturent pas en euros, leurs montants doivent être convertis.
AMAZON_MARKETPLACES = [
...
# Canaux volontairement hors périmètre de la feuille REPORT AMAZON VENTES
# (elle ne comporte que les 8 pays de AMAZON_MARKETPLACES). Leur CA est
# journalisé mais ne déclenche pas d'alerte : ce n'est pas une ventilation
# ratée, c'est du hors-scope assumé. Déplacer une entrée vers
# AMAZON_MARKETPLACES suffit à la faire entrer dans le périmètre.
AMAZON_IGNORED_CHANNELS = {"amazon.co.uk", "amazon.ie", "non-amazon"}


def _parse_amazon_orders_report(tsv: str, date_str: str) -> dict:
    """Ventile un rapport plat Amazon en CA HT par pays.

    Une ligne = un article. On calcule le HT article par article :
      - si item-tax > 0 → item_price - item_tax (TVA réellement collectée)
      - sinon (LIC, export Suisse, TVA 0) → item_price / (1 + taux du pays)
    Les frais de port suivent la même règle. Les montants en devise étrangère
    sont convertis en EUR au taux BCE du jour, d'après la colonne currency.
    ...
    for line in lines[1:]:
        ...
        # un canal inconnu, où elle ne doit pas déclencher l'alerte.
        if col(cols, "order-status").lower() == "cancelled":
            cancelled += 1
            continue
        ...
        vat = mk["vat"]
        ht  = 0.0
        for price_key, tax_key in (("item-price", "item-tax"), ("shipping-price", "shipping-tax")):
            try:
                price = float(col(cols, price_key) or 0)
            except ValueError:
                continue
            if not price:
                continue
            try:
                tax = float(col(cols, tax_key) or 0)
            except ValueError:
                tax = 0.0
            ht += (price - tax) if tax > 0 else (price / (1 + vat))

        # La devise est lue sur la ligne, pas déduite du pays : un marketplace
        # peut facturer dans une autre devise que celle attendue.
        cur_row = col(cols, "currency").upper() or mk["currency"]
        if cur_row != "EUR":
            rate = get_fx_rate_to_eur(cur_row, date_str)
            if rate is None:
                raise AmazonReportUnreadable(
                    f"taux {cur_row}→EUR indisponible pour {date_str}"
                )
            ht *= rate
    ...
    for m in AMAZON_MARKETPLACES:
        ...
        log.info(f"[Amazon {name}] {len(orders[name])} commande(s) → CA HT = {results[name]}")
    return results
...
def compare_amazon_methods(date_api: str) -> int:
    ...
    tot_r = tot_l = 0.0
    ecarts = 0
    # Un total n'a de sens que si TOUS les pays ont une valeur : sommer en
    # ignorant les None produirait un total qui ressemble à une vraie mesure.
    total_r_valide = all(rapport.get(m["name"]) is not None for m in AMAZON_MARKETPLACES)
    total_l_valide = all(legacy.get(m["name"]) is not None for m in AMAZON_MARKETPLACES)
    ...
    for m in AMAZON_MARKETPLACES:
        ...
        if isinstance(l, (int, float)):
            tot_l += l
        print(f"  {name:<12} {fmt(r):>14} {fmt(l):>14} {ecart:>14}")
    ...
    print(f"  {'-'*12} {'-'*14} {'-'*14} {'-'*14}")
    print(f"  {'TOTAL':<12} {aff_r:>14} {aff_l:>14} {aff_d:>14}")
    print(f"{'='*64}")
    ...
    if all(v is None for v in rapport.values()):
        ...
        print("      Écarts attendus : commandes annulées (exclues du rapport, comptées")
        print("      par l'Orders API) et conversions SEK/PLN (rapport seulement).")
    else:
